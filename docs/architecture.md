...
flowchart LR
    ...
    App --> Chain[web3 readonly]
    App --> Relay[web3/relayer gasless]
    Public --> HTTP[internal/transport]
    Trading --> HTTP
    HTTP --> PM[Polymarket API]
    Stream --> Feeds[WebSocket / RTDS / Polygon WSS]
    Chain --> RPC[Polygon RPC]
    Relay --> Chain
    Relay --> Relayer[Polymarket Relayer]
    Chain --> Sign[signing]
    Trading --> Chain
...
`web3` 负责 RPC 池、钱包地址解析和只读余额/授权查询；`web3/relayer` 在其之上负责 gasless 交易编排（批量构造、签名、提交中继）；`signing` 负责本地密钥与协议签名。私钥不应进入 `types`、日志或业务缓存，持有 signer 的客户端结束使用后必须 `Close()`。
...
- `web3/relayer` 依赖 `web3`；`web3` 不依赖 `web3/relayer`。
...
- 新钱包类型：先在 `types.SignatureType` 和 `signing` 建立明确语义，再接入 `web3`/`web3/relayer`/`clob`。
