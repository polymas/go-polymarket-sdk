...
> 本文面向资金准备、授权与 gasless 链上流程。第一次使用 SDK 请先阅读[快速开始](getting-started.md)。
...
仓库的链上示例和诊断命令可手动核对每一步：

| 用途 | 命令 |
|---|---|
| 看当前所有 V2 状态 | `go run ./cmd/diagnostics/v2-allowance-check` |
| 自检（不发 tx） | `go run ./examples/onchain/v2-ensure status` |
| 缺啥补啥 | `go run ./examples/onchain/v2-ensure ensure` |
| auto-claim 状态 | `go run ./examples/onchain/v2-auto-claim status` |
| auto-claim 开关 | `go run ./examples/onchain/v2-auto-claim enable\|disable` |
| pUSD 余额查询 | （直接调 `gasless.GetPUSDBalance()`，无独立 example） |
| split / merge / redeem | `go run ./examples/onchain/v2-split-merge-redeem <op> <conditionId> <args>` |
