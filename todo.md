...
- `roundNormal` 只按“小数位数”舍入，不校验价格是不是 tick 的整数倍。例如 tick=`0.005` 时，`0.007` 仍可能被保留为三位小数，但它不在合法网格上。
- `getDecimalPlacesFromTickSize(0.0025)` 会返回 3，实际需要保留 4 位。
- maker/taker amount 的精度配置同样只按阈值分档，无法正确表达这些非 10 的幂次 tick。
- 关键金额路径广泛使用 `float64`，存在二进制浮点误差。

风险：订单签名里的整数金额可能与官方客户端不同，导致本地看似合法、服务端拒绝，或价格被意外改到另一档。

建议验收：

- [ ] 使用十进制定点数或整数运算实现 `price % tick == 0`，不要用 `float64` 直接判等。
- [ ] 对全部官方 tick 建立表驱动测试，并与官方 TS/Python/Rust SDK 生成的 maker/taker amounts 做 golden test。
- [ ] 非网格价格默认返回 typed validation error；只有调用方明确选择 rounding policy 时才自动修正，并返回修正后的值。
- [ ] 覆盖 BUY/SELL、边界价格、半格价格、极小/极大 size。
